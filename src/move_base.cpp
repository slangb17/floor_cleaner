...
bool cornerPassed = false;
int counter = 0;

clock_t old_time = 0;
clock_t turning_time = 0;
...
void scan_callback(const sensor_msgs::LaserScan::ConstPtr &scan_msg)
{
  ...
  float laser_ranges_min = laser_msg.range_min;
  float laser_ranges_max = laser_msg.range_max;

  // The sensors ranges are divided into three sections: left, center, and right
  float laser_ranges_left = 0.0f;
  float laser_ranges_center = 0.0f;
  float laser_ranges_right = 0.0f;
  ...
  if (cornerFound != true)
  {
    ...
    if (laser_ranges_min < 0.3)
    {
      velocity_msg.linear.x = 0.01;
      //wallDetected = true;
    }
    else if (laser_ranges_min < 0.65)
    {
    ...
    if (wallDetected == true && (wallToTheLeft == true && laser_ranges_right > laser_ranges_center) ||
        (wallToTheLeft == false && laser_ranges_left > laser_ranges_center))
    {
...
void corner_turner()
{
  ...
  if ((wallToTheLeft == true && laser_ranges_left > 0.65) && wallToTheLeft == false && laser_ranges_right > 0.65)
  {
    cornerPassed = true;
...
int main(int argc, char **argv)
{
  ...
  // Subscriber
  ros::Subscriber laser_scan = n.subscribe("/scan", 640, scan_callback);
